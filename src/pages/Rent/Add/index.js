...
import { List, InputItem, Picker, ImagePicker, TextareaItem, Flex, Modal, Toast } from 'antd-mobile';
import HousePackge from '../../../components/HousePackage'
import { userHouse, houseImage } from '../../../utils/api'
...
class RentAdd extends PureComponent {
    ...
    constructor(props) {
        ...
        if (query) {
            // 有小区信息数据，存储到状态中
            community.name = query.name
            community.id = query.id
        }
 ...
 */
    addHouse = async () => {
        ...
        if (tempSlides.length > 0) {
            ...
            const { data } = await houseImage(form)
        ...
        const { data } = await userHouse(params)
        if(data.status===200){
            alert('提示', '发布房源成功', [
                {
                    text: '确定',
                    onPress: async () => this.props.history.go('/rent')
                }
            ])
        }else{
    ...
    render() {
        ...
        console.log(community, price, roomType, floor, oriented, description, tempSlides, title, size, supporting)
        return (
            ...
            <div className={styles.root}>
                ...
                <List renderHeader={() => '房源信息'} className={styles.header}>
                    <Item extra={community.name || '小区名称'} arrow="horizontal" onClick={() => { this.props.history.push('/search') }}> 小区名称</Item>
                    <InputItem
                        placeholder="请输入租金/月"
                        extra="￥/月"
                        value={price}
                    ...
                    >
                        <Item arrow="horizontal" >  楼&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;层</Item>
                    </Picker>
                    <Picker
                        data={orientedData}
                        value={[oriented]}
